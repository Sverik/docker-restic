FROM golang:1.10.2-alpine
MAINTAINER Sven-Erik Petermann <sverik@gmail.com>

ENV RESTIC_VERSION="0.9.1"

# Cleanup params
ENV RESTIC_CLEANUP_KEEP_DAILY=7
ENV RESTIC_CLEANUP_KEEP_WEEKLY=5
ENV RESTIC_CLEANUP_KEEP_MONTHLY=12
ENV RESTIC_CLEANUP_KEEP_YEARLY=75

# Default interval times can be set in cron expression
# Fire at 03:15 every day
ENV CRON_BACKUP_EXPRESSION="15   3  *   *   *"
# Fire at 00:15 on the first day of every month
ENV CRON_CLEANUP_EXPRESSION="15  0  0   *   *"

# Install the items
RUN apk update \
  && apk add ca-certificates wget gnupg \
  && update-ca-certificates
RUN wget -O /tmp/restic-${RESTIC_VERSION}.tar.gz "https://github.com/restic/restic/releases/download/v${RESTIC_VERSION}/restic-${RESTIC_VERSION}.tar.gz" \
  && cd /tmp \
  && tar -xf /tmp/restic-${RESTIC_VERSION}.tar.gz -C /tmp/
RUN cd /tmp/restic-${RESTIC_VERSION} \
  && go run build.go \
  && mv restic /go/bin/restic \
  && rm -rf /tmp/restic* /var/cache/apk/*

# Script and config
ADD ./target/* /restic/

# Start the process
CMD /restic/start_cron.sh

